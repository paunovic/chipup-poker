...
type
  ...
  public
    ...
    procedure MergeFrom(const AFrom: TPB_RpcMessage);
    procedure Clear;
...
procedure TPB_RpcMessage.MergeFrom(const AFrom: TPB_RpcMessage);
begin
  if AFrom.has_MethodId then
    SetMethodId(AFrom.MethodId);
  if AFrom.has_DataSize then
    SetDataSize(AFrom.DataSize);
  if AFrom.has_Token then
    SetToken(AFrom.Token);
end;
