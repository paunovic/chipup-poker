...
function sendAuthEmail(userid,authcode,email,fail1,fail2,sucess) {
	...
	test.sendMail(email,'From: clever@angeldsis.com\r\nSubject: test\r\n\r\nConfirmation link: '+link,function cb(err,ret) {
		console.log('cb',err,ret);
		if (err) {
	...
	case 2: // in the main lobby
		...
		case codes.scCreateClub:
			...
			allClubs.findOne({name:{$regex:new RegExp('^'+params.name+'$','i')}},function (err,row) {
				...
				var doc = {is_private:params.is_private,password:params.password,name:params.name, owner:this.userid, chips:100000,rake:params.rake};
	...
	allGames.update({_id:this.id},{$set:{state2:'gsClosed'}},function (err,ret) {
		...
		if (count == 0) {
			...
			allClubs.findOne({_id:this.obj.clubid},function (err,club) {
				...
				for (var x=0; x<club.members.length; x++) {
					conn2 = activeUsers[club.members[x]];
					if (!conn2) continue;
...
Game.prototype.deal = function deal(cb,config) {
	...
	getNextSequence('handHistory',function (seq) {
		this.handid = seq;
		allGames.update({_id:this.id},{$set:{lasthandid:seq}},function (err,res){});
...
Game.prototype.doWin = function (cb,extradelay) {
	...
	for (var y=0; y<this.pots.length; y++) {
		...
		totalrake += rake;
		console.log('rake:%d pot:%j',rake,pot);
			...
			this.saveHistory(function () {
				for (var x=0; x<this.balance_changes.length; x++) {
					...
					if (this.balance_changes[x] != 0) {
						...
						hack.call(this,doc,mods,{gameid:this.obj._id,userid:this.seats[x].userid});
					}
				}
				cb(events,0);
			}.bind(this));
