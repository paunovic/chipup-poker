...
function evalHandSuit(table,hand,suit,tableraw,handraw) {
	var hunique = {},tunique = {};
	for (var x1 = 0; x1 < 4; x1++) {
		if (suit != (handraw[x1]%4)) continue;
		for (var x2=0; x2<4; x2++) {
			if (suit != (handraw[x2]%4)) continue;
			if (x2 <= x1) continue;
			hunique[format1(hand[x1])+format1(hand[x2])] = [hand[x1],hand[x2]];
		}
	}
	for (var x1=0; x1<5; x1++) {
		if (suit != (tableraw[x1]%4)) continue;
		for (var x2=0; x2<5; x2++) {
			if (suit != (tableraw[x2]%4)) continue;
			if (x2 <= x1) continue;
			for (var x3=0; x3<5; x3++) {
				if (suit != (tableraw[x3]%4)) continue;
				if (x3 <= x2) continue;
				tunique[format1(table[x1])+format1(table[x2])+format1(table[x3])] = [ table[x1],table[x2],table[x3]];
			}
		}
